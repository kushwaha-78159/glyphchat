...
import {
  Bell, Check, ChevronDown, Copy, Hash, Info, LockKeyhole, Menu, Paperclip,
  Phone, Plus, Search, Send, Settings2, Smile, Sparkles, Video, X,
} from "lucide-react";
...
export default function Home() {
  ...
  const [composerMode, setComposerMode] = useState<"plain" | "symbols">("plain");
  ...
  function sendMessage() {
    ...
    const plainText = composerMode === "plain" ? draft : decodeSymbols(draft);
    ...
    <main className="workspace">
      ...
      <section className="chat-panel">
        ...
        <div className={`chat-body ${activePanel === "alphabet" ? "mobile-hide-chat" : ""}`}>
          ...
          <div className="messages">{messages.map((message) => <div key={message.id} className={`message-row ${message.mine ? "mine" : ""}`}>{!message.mine && <Avatar initials="MC" color="coral" size="sm" />}<div className="message-stack"><div className="message-bubble"><p>{message.text}</p><div className="encoded-line"><span>{message.encoded}</span><button aria-label="Copy encoded message" onClick={() => copyValue(message.encoded, String(message.id))}>{copied === String(message.id) ? <Check size={13} /> : <Copy size={13} />}</button></div></div><div className="message-meta">{message.time}{message.mine && <><span>·</span><Check size={13} className="read-check" /><Check size={13} className="read-check second" /></>}</div></div></div>)}</div>
        </div>
        <div className="composer-wrap"><div className="composer-mode"><span>Writing in</span><button onClick={() => setComposerMode((mode) => mode === "plain" ? "symbols" : "plain")}><span className={`mode-dot ${composerMode}`} />{composerMode === "plain" ? "Plain text" : "Glyph symbols"}<ChevronDown size={14} /></button><span className="mode-hint">{composerMode === "plain" ? "will be encoded on send" : "symbols will be decoded on send"}</span></div><div className="composer"><button className="composer-icon" aria-label="Attach file"><Paperclip size={19} /></button><input value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") sendMessage(); }} placeholder={composerMode === "plain" ? "Write a message…" : "Tap a glyph below…"} /><button className="composer-icon" aria-label="Add emoji"><Smile size={19} /></button><button className="send-button" aria-label="Send message" onClick={sendMessage}><Send size={18} /></button></div><div className="symbol-keyboard"><div className="keyboard-head"><span><Hash size={14} /> Glyph keyboard</span><span>26 symbols · tap to insert</span></div><div className="keys">{alphabet.map(([letter, symbol]) => <button key={letter} className="key" title={`${letter} → ${symbol}`} onClick={() => setDraft((value) => value + (composerMode === "plain" ? letter : symbol))}><span>{symbol}</span><small>{letter}</small></button>)}</div></div></div>
      </section>
